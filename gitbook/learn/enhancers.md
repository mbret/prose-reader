...
enhancer does so for urls, and the KOReader enhancer for xpointers. Widened
options no longer satisfy a constraint on the narrower ones, so an enhancer
applied after yours should constrain its options only to what it reads, such as
`Pick<CreateReaderOptions, "getRenderer">`.
