...
type SnapNavigation = {
  type: `snap`
  data: {
    from: ViewportNavigationEntry
    to: ViewportNavigationEntry
      ...
      if (delta) {
        ...
        navigation = navigator.wrapPositionWithSafeEdge({
          ...
          y:
            pageTurnDirection === `horizontal`
              ? 0
              : movingLastPosition.y - correctedY,
        })
    ...
    (
      ...
      if (spineItemHasChanged) {
        if (allowSpineItemChange) {
          if (
            spineItemManager.comparePositionOf(
              newSpineItem,
              currentSpineItem,
            ) === `before`
          ) {
  ...
  const snapNavigation$ = navigationTriggerSubject$.pipe(
    ...
    switchMap(
      ([
        ...
        const triggerPercentage = movingForward
          ? 1 - navigationSnapThreshold
          : navigationSnapThreshold
        const triggerXPosition =
          pageTurnDirection === `horizontal`
            ? to.x + context.state.visibleAreaRect.width * triggerPercentage
            : 0
        const triggerYPosition =
          pageTurnDirection === `horizontal`
            ? 0
            : to.y + context.state.visibleAreaRect.height * triggerPercentage
        const midScreenPositionSafePosition =
          navigator.wrapPositionWithSafeEdge({
            x: triggerXPosition,
            y: triggerYPosition,
          })
        const finalNavigation = navigator.getNavigationForPosition(
          midScreenPositionSafePosition,
        )
        const lastUserExpectedNavigation =
          getLastUserExpectedNavigation(finalNavigation)
