import { type CfiPart, type CfiRange, type ParsedCfi, parse, unwrapCfi } from "./parse"
...
export function resolve(
  ...
  try {
    ...
    if (unwrappedCfi.match(/\/\d+\/\d+\[[^\]]+\]\/\d+:\d+\[[a-z]\]/)) {
      const match = unwrappedCfi.match(/\/\d+\/\d+\[([^\]]+)\]\/\d+:(\d+)\[([a-z])\]/);
      if (match) {
...
function resolveRange(
  ...
  try {
    ...
    if (!(startResult.node instanceof Node) || !(endResult.node instanceof Node)) {
      if (throwOnError) {
    ...
    if (typeof domRange.setStart === 'function') {
      const startOffset = startResult.offset !== undefined 
        ? (Array.isArray(startResult.offset) ? startResult.offset[0] : startResult.offset) 
        : 0;
        
      const endOffset = endResult.offset !== undefined 
        ? (Array.isArray(endResult.offset) ? endResult.offset[0] : endResult.offset)
        : 0;
    ...
    if (asRange) {
      ...
      if (document.createRange) {
        ...
        if (lastPart && lastPart.offset !== undefined) {
          const offset = Array.isArray(lastPart.offset) ? lastPart.offset[0] : lastPart.offset;
...
/**
 * Get last properties from a CFI part
 */
function getLastProperties(parts: CfiPart[]): Partial<CfiPart> {
