...
describe("EPUB CFI Resolve", () => {
  ...
  describe("resolve", () => {
    it("should resolve a simple CFI to a node", () => {
      ...
      expect(result.node).toBe(document.getElementById("p1"));
    });
    ...
    it("should resolve a CFI with element ID", () => {
      ...
      expect(result.isRange).toBe(false);
      expect(result.node).toBe(document.getElementById("p2"));
    });
    
    it("should resolve a CFI range", () => {
      const cfi = "/4/2[chap01ref]/1:5,/4/2[chap01ref]/2[chap02ref]/1:10";
      const result = resolve(cfi, document, { asRange: true });
      
      expect(result.isRange).toBe(true);
      expect(result.node instanceof Range).toBe(true);
      
      const range = result.node as Range;
      expect(range.startContainer).toBe(document.getElementById("p1"));
      expect(range.startOffset).toBe(5);
      expect(range.endContainer).toBe(document.getElementById("p2"));
    ...
    it("should throw on error if throwOnError is true", () => {
      ...
      expect(() => {
        resolve(cfi, document, { throwOnError: true });
      }).toThrow();
    });
  });
  ...
  describe("resolveParsed", () => {
    it("should resolve a parsed CFI to a node", () => {
      ...
      const result = resolveParsed(parsed, document);
    ...
    it("should resolve a parsed CFI range", () => {
      const parsed = parse("/4/2[chap01ref]/1:5,/4/2[chap01ref]/2[chap02ref]/1:10");
      const result = resolveParsed(parsed, document, { asRange: true });
